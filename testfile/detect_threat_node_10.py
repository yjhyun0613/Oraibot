...
- TF 변환 오류 해결: 'robot5' 네임스페이스 적용 및 상대경로 토픽 통합
=============================================================================
"""
...
class SmartVisionSafetyNode(Node):
    def __init__(self):
        # 🚨 [수정 1] 노드 자체를 'robot5' 네임스페이스 안으로 넣습니다!
        # 이제 TF_Listener가 알아서 '/robot5/tf'를 구독하게 됩니다.
        super().__init__('smart_vision_safety_node', namespace='robot5')
        ...
        # 🚨 [수정 2] 네임스페이스가 적용되었으므로 앞에 붙던 '/robot5/'를 전부 제거하여 깔끔하게 만듭니다.
        self.map_sub = self.create_subscription(OccupancyGrid, 'map', self.map_callback, map_qos) 
        self.scan_sub = self.create_subscription(LaserScan, 'scan', self.scan_callback, 10)
        self.camera_sub = self.create_subscription(Image, 'oakd/rgb/image_raw', self.image_callback, 10)
        self.goal_sub = self.create_subscription(PoseStamped, 'goal_pose', self.goal_callback, 10)
        self.resume_sub = self.create_subscription(String, 'resume_cmd', self.resume_callback, 10)
    ...
    def scan_callback(self, msg):
        ...
        try:
            ...
            # 🚨 [새로 추가된 부분] 라이다 이름표에 'robot5/' 가 안 붙어있으면 강제로 붙여줍니다!
