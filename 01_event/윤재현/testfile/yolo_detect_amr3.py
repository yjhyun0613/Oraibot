...
class YoloNavAlertNode(Node):
    ...
    def send_alert_once(self, img, distance):
        """이미지 전송 시 검은 화면 방지를 위한 강제 복사 및 타입 확인 [cite: 2026-04-16]"""
        msg = String()
        ...
        try:
            if img is not None and img.size > 0:
                # 1. 원본 데이터를 보존하기 위해 깊은 복사 수행 [cite: 2026-04-16]
                alert_frame = np.array(img, copy=True)
                
                # 2. 전송 직전 이미지 타입 확인 (uint8인지 체크)
    ...
    def detection_loop(self):
        while not self.should_shutdown:
            ...
            for r in results:
                ...
                for box in r.boxes:
                    ...
                    if current_dist > 0:
                        ...
                        # 텍스트만 표시 (배경 없이 가독성 위해 외곽선 효과) [cite: 2026-04-16]
            ...
            # 제어 및 알람 로직
            # 제어 및 알람 로직
            if detected_in_range and min_dist < 1.0: # 3.0을 1.0으로 변경
            ...
            elif not detected_in_range or min_dist >= 3.5: # 5.0을 3.5로 변경
                if self.nav_paused:
